...
class TestUser:
    ...
    def test_reading_list(self):
        books = [Book(1,"1"),Book(2,"2"),Book(3,"3"),Book(4,"4"),Book(5,"5"),Book(6,"6"),Book(7,"7"),Book(8,"8"),Book(9,"9"),Book(10,"10"),Book(1111,"NewBook")]
        user2 = User('Martin', 'p90')
        user2.add_book_to_reading_list(books[0])
        assert books[0] in user2.reading_list
        for book in books:
            user2.add_book_to_reading_list(book)
        assert books[0] not in user2.reading_list
        assert books[-1] in user2.reading_list
        user2.remove_book_from_reading_list(books[-1])
        assert books[-1] not in user2.reading_list
...
class TestBooksJSONReader:
    ...
    def test_read_books_from_file(self, read_books_and_authors):
        ...
        assert str(dataset_of_books[19]) == "<Book Visión ciega, book id = 13575839>"
    ...
    def test_read_books_from_file_and_check_other_attributes(self, read_books_and_authors):
        ...
        assert "El 13 de febrero de 2082" in dataset_of_books[19].description
    ...
    def test_read_books_from_file_special_characters(self, read_books_and_authors):
        dataset_of_books = read_books_and_authors
        assert  dataset_of_books[16].title == "續．星守犬"
