...
pyinstaller .\pywebview_installed.py --add-data "library;library" --add-data "sql_app.db;." --noconfirm --clean --name $applicationName -hide-console --contents-directory "." --icon "library\static\favicon.ico"
# # Copy cv2 dependency from virtual environment to application folder because pyinstaller does not package it
# Copy-Item -Path "$venvPackagePath\cv2\*" -Destination "$applicationFolder" -Recurse
iscc .\package.iss
